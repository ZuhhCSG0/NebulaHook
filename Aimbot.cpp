        ...
        if( !latest_record.needs_extrapolation ) {
            ...
            for( auto HitboxID : GetHitboxesToScan( pTarget ) ) {
                ...
                for( int k = 0; k < Points.size(); k++ ) {
                    ...
                    {
                        if( RagebotElements.Ragebot_Autowall->Checked && g_Autowall->PenetrateWall( pTarget, vPoint ) ) {
...
void CAimbot::Run()
{
    ...
    if( target_index != -1 && current_aim_position != Vector( 0, 0, 0 ) && pTarget ) 
	{
        ...
        if( RagebotElements.Ragebot_Type->Checked == 0 )
            g_pEngine->SetViewAngles( G::UserCmd->viewangles );



		if( RagebotElements.Ragebot_AutoShoot->Checked )
		{
            auto HitChance = []( ) -> float
            {
				//	c_weapon* pWeapon = (c_weapon*)I::EntList->GetClientEntityFromHandle(pLocal->GetActiveWeaponHandle());
				float hitchance = 101;
				if (!G::LocalPlayer->GetWeapon()) return 0;
				if (RagebotElements.Ragebot_Hitchance_Slider->value >= 1)
				{//Inaccuracy method
            ...
            if( RagebotElements.Ragebot_Hitchance_Slider->value == 0 || (RagebotElements.Ragebot_Hitchance_Slider->value * 1.5 <= HitChance()) ) {
