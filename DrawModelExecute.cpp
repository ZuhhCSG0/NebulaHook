...
void __fastcall Hooks::scene_end(void* thisptr, void* edx) {
	...
	static IMaterial* OpenFlatW = g_MaterialHelper->CreateMaterial(true, false, true);
	static IMaterial* CoveredFlatW = g_MaterialHelper->CreateMaterial(true, true, true);
	...
	static IMaterial* Chrome = g_MaterialHelper->CreateMaterial("$envmap env_cube");
	static IMaterial* glass = g_MaterialHelper->CreateMaterial(g_pMaterialSystem->FindMaterial("models/inventory_items/cologne_prediction/cologne_prediction_glass", "Other textures"));

	int ChamsStyle = VisualElements.Visual_Chams_Style->SelectedIndex;

	IMaterial *mat = ChamsStyle == 1 || ChamsStyle == 2 ? g_MaterialHelper->OpenLit : g_MaterialHelper->OpenFlat;
		...
		for (int i = 1; i < g_pGlobals->maxClients; ++i) 
		{
			...
			if (G::LocalPlayer)
			{
				if (G::LocalPlayer->isAlive())
				{
					...
					{
						if (g_pInput->m_fCameraInThirdPerson && VisualElements.Visual_Misc_FChams->Checked)
						{
							float flColor[3];
					...
					else
					{
						if (VisualElements.Visual_Chams_Active->Checked)
						{
							...
							if (VisualElements.Visual_Chams_EnemyOnly->Checked && ent->GetTeamNum() == G::LocalPlayer->GetTeamNum())
								continue;
							...
							if (ent->GunGameImmunity())
								alpha /= 2.f;

							if (ent->GetTeamNum() == 3)
							{
								flColor2[0] = getR(ColorElements.Color_CT_Invisible_Cham->color) / 255.f;
								flColor2[1] = getG(ColorElements.Color_CT_Invisible_Cham->color) / 255.f;
								flColor2[2] = getB(ColorElements.Color_CT_Invisible_Cham->color) / 255.f;
							}
							else
							{

								flColor2[0] = getR(ColorElements.Color_T_Invisible_Cham->color) / 255.f;
								flColor2[1] = getG(ColorElements.Color_T_Invisible_Cham->color) / 255.f;
								flColor2[2] = getB(ColorElements.Color_T_Invisible_Cham->color) / 255.f;
							}
							...
							if (ent->GetTeamNum() == 3)
							{
								flColor2[0] = getR(ColorElements.Color_CT_Visible_Cham->color) / 255.f;
								flColor2[1] = getG(ColorElements.Color_CT_Visible_Cham->color) / 255.f;
								flColor2[2] = getB(ColorElements.Color_CT_Visible_Cham->color) / 255.f;
							}
							else
							{

								flColor2[0] = getR(ColorElements.Color_T_Visible_Cham->color) / 255.f;
								flColor2[1] = getG(ColorElements.Color_T_Visible_Cham->color) / 255.f;
								flColor2[2] = getB(ColorElements.Color_T_Visible_Cham->color) / 255.f;
							}
