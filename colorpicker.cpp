...
void MColorPicker::Draw()
{
	...
	g_pRender->Text((char*)this->name.c_str(), position.x + 45, this->position.y, lefted, g_pRender->Fonts.menu_control, true, WHITE(alpha), BLACK(alpha));

	g_pRender->FilledBoxOutlined(position.x, position.y, 40, 10, this->color, BLACK(alpha));

	if (m_pMenu->MainWindow->Clicked(Vector(position.x, position.y), Vector(40, 10)))
		is_open = true;

	if (is_open)
	{
		DrawColorPicker(position.x, position.y + 15, g_pRender->device);
		g_pRender->BorderedBox(position.x, position.y + 15, 256, 256, D3DCOLOR_ARGB(alpha, 200, 0, 0));
		if (m_pMenu->MainWindow->bMouse1pressed && m_pMenu->MainWindow->InBounds(Vector(position.x, position.y + 15), Vector(256, 256)))
		{
			this->color = GetPickedColor(m_pMenu->MainWindow->Cur.x - position.x, m_pMenu->MainWindow->Cur.y - position.y + 15);
		}

		if (m_pMenu->MainWindow->bMouse1released && !m_pMenu->MainWindow->InBounds(Vector(position.x, position.y), Vector(256, 256 + 15)))
			is_open = false;
	}
}