...
void MCheckbox::Draw()
{
	...
	if (this->Checked)
		g_pRender->GradientBoxOutlined(this->position.x, this->position.y + 2, this->size.x, this->size.y, 1,D3DCOLOR_ARGB(alpha, 255, 0, 0), D3DCOLOR_ARGB(alpha, 200, 0, 0), BLACK(alpha),true);
	else
	{
		
			if (bHovering)
			{
				g_pRender->FilledBoxOutlined(this->position.x, this->position.y + 2, this->size.x, this->size.y, D3DCOLOR_ARGB(alpha, 200, 0, 0), BLACK(alpha));
				//g_pRender->GradientBoxOutlined(this->position.x, this->position.y + 2, this->size.x, this->size.y, 1, D3DCOLOR_ARGB(alpha, 0, 122, 174), D3DCOLOR_ARGB(alpha, 0, 92, 144), BLACK(alpha), false);
			...
			{
				g_pRender->GradientBoxOutlined(this->position.x, this->position.y + 2, this->size.x, this->size.y, 1, D3DCOLOR_ARGB(alpha, 45, 45, 45), D3DCOLOR_ARGB(alpha, 35, 35, 35), BLACK(alpha), false);
			}
