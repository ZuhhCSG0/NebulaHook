...
void CESP::DrawPlayer(CBaseEntity* pPlayer, CBaseEntity* pLocalPlayer)
{
	...
	if (GameUtils::WorldToScreen(m_position, footpos) && GameUtils::WorldToScreen(m_top_position, headpos))
	{
		...
		if (VisualElements.Visual_Player_Health->Checked)
		{
			...
			g_Draw.FillRGBA(x - (12), y + health*height / 100, w, pPlayer->GetHealth()*height / 100, hr, hg, hb, 255);

			g_Draw.Box(x - (12), y, w , height, 0, 0, 0, 255);
			...
			if (pPlayer->GetHealth() < 100)
			{
				...
				g_Draw.StringA(g_Draw.font_esp, false, x - 12 - g_Draw.getWidht(hp, g_Draw.font_esp), y + health*height / 100 - 12, 200, 200, 200, 255, "%i", pPlayer->GetHealth());
			}
			int step_height = height / 10;
			for (int i = 1; i < 10; i++)
			{
				g_Draw.LineRGBA(x - 12, y + i*step_height, x - 8, y + i*step_height, 0, 0, 0, 255);
			}
		}
		int bot_add = 0;
		int top_add = 0;
		if (VisualElements.Visual_Player_Name->Checked)
		{
			...
			if (G::ResolverMode[pPlayer->GetIndex()] == 0)
				g_Draw.StringA(g_Draw.font_esp, false, x + width + 10, y, 200, 200, 200, 255, "Resolver Mode : LBY");
			else if (G::ResolverMode[pPlayer->GetIndex()] == 1)
				g_Draw.StringA(g_Draw.font_esp, false, x + width + 10, y, 200, 200, 200, 255, "Resolver Mode : LBY Breaker Predict");
			else if (G::ResolverMode[pPlayer->GetIndex()] == 2)
				g_Draw.StringA(g_Draw.font_esp, false, x + width + 10, y, 200, 200, 200, 255, "Resolver Mode : Bruteforce");



			std::string shots = "Shots = " + std::to_string(G::Shots[pPlayer->GetIndex()]);
			g_Draw.StringA(g_Draw.font_esp, false, x + width + 10, y + 20, 200, 200, 200, 255, shots.c_str());
		}
			...
			if (hWeapons)
			{
				...
				if (pMainWeapon)
				{
					...
					if (VisualElements.Visual_Player_Weapon_Ammo->Checked && pMainWeapon->Clip1() != -1 && pMainWeapon->GetWeaponType() != WEAPONCLASS::WEPCLASS_INVALID)
					{
						...
						s1.append(XorStr(" | "));
						s1.append(to_string(pMainWeapon->get_full_ammo()));
						s1.append(XorStr(" ) "));
					}
					if(VisualElements.Visual_Player_Weapon->Checked)
...
void CESP::DrawHostage(CBaseEntity* pHostage)
{
	...
	DWORD col = ColorElements.Color_Weapon->color;
...
void CESP::DrawDroppedWeapon(CBaseCombatWeapon* pWeapon)
{
	...
	if (!sCurWeapon.empty() && sCurWeapon.find(XorStr("w_")) != std::string::npos)
	{
		...
		name.append(XorStr(" ( "));
		name.append(to_string(Weapon->Clip1()));
		name.append(XorStr(" | "));
		name.append(to_string(Weapon->get_full_ammo()));
		name.append(XorStr(" ) "));
...
void CESP::Loop()
{
	...
	if (G::LocalPlayer && G::LocalPlayer->GetHealth() > 0 && G::MainWeapon)
	{
		if (G::MainWeapon->IsGrenade())
		{
			...
			if (VisualElements.Visual_Misc_GrenadePrediction->Checked)
	...
	for (int i = 0; i < GlowObjectManager->size; i++)
	{
		...
		if (VisualElements.Visual_CBaseEntity_Glow->Checked)
		{
...
void CESP::DrawScope(CBaseEntity* pLocalPlayer)
{
	if (VisualElements.Visual_Misc_NoScope->Checked)
	{
		if (pLocalPlayer && pLocalPlayer->GetHealth() > 0)
		{
			...
			{
				g_Draw.LineRGBA(MidX, MidY - 3000, MidX, MidY + 3000, 0, 0, 0, 255);
				g_Draw.LineRGBA(MidX - 3000, MidY, MidX + 3000, MidY, 0, 0, 0, 255);
			}
		}
	}
}