...
c_backtrack* g_backtracking = new c_backtrack();
...
int CBacktrackHelper::GetEstimateServerTickCount()
{
	...
	if (!nci) return g_pGlobals->tickcount;
	...
	return g_pGlobals->tickcount + LagTime + max(g_pGlobals->frametime / g_pGlobals->interval_per_tick, 1);
}
...
void CBacktrackHelper::UpdateBacktrackRecords(CBaseEntity* pPlayer) 
{
	...
	bool breaks_lagcomp = v.Length() > 64.f;
	...
	new_record.m_angEyeAngles = pPlayer->GetAbsAngles();
...
//	new_record.m_flAnimTime = pPlayer->GetAnimationTime();
	...
	int sequence = g_pModelInfo->GetStudioModel(pPlayer->GetModel())->pSeqdesc(pPlayer->GetSequence())->activity;
...
void c_backtrack::UpdateRecords(CBaseEntity* pPlayer)
{
	//grab each entities index
	int p_index = pPlayer->GetIndex();

	for (int j = s_player_record[p_index].records.size() - 1; j >= 0; j--)
	{
		float lerptime = g_BacktrackHelper->GetLerpTime();
		float desired_time = s_player_record[p_index].records.at(j).m_flSimulationTime + lerptime;
		float estimated_time = g_BacktrackHelper->GetEstimateServerTime();
		...
		if (fabs(deltaTime) > 0.2f)
			s_player_record[p_index].records.erase(this->s_player_record[p_index].records.begin() + j);
	}

	//record check
	if (s_player_record[p_index].records.size() > 0 && pPlayer->GetSimulationTime() == s_player_record[p_index].records.back().m_flSimulationTime) //already got such a record
		return;

	if (s_player_record[p_index].records.size() > 0 && s_player_record[p_index].records.back().m_flSimulationTime > pPlayer->GetSimulationTime())//Invalid lag record, maybe from diffrent game?
	{
		s_player_record[p_index].records.clear();
		return;
	}
	...
	//grab origin delta
	Vector d_origin = pPlayer->GetAbsOrigin() - old_origin[p_index];

	old_origin[p_index] = pPlayer->GetAbsOrigin();

	//if origin delta length is greater than 64, they are breaking lag compensation
	if (d_origin.Length() > 64.f)
		return;
	...
	//add stuff to record
	record.m_angEyeAngles = pPlayer->GetAbsAngles();
	record.m_flSimulationTime = pPlayer->GetSimulationTime();
	record.m_vecOrigin = pPlayer->GetAbsOrigin();
	record.m_vecVelocity = pPlayer->GetVelocity();

	//store this record for restoring
	s_player_record[p_index].records.push_back(record);

	printf("nibba 4");
}

void c_backtrack::RestoreRecords(CBaseEntity* pPlayer)
{
	//grab player index
	int p_index = pPlayer->GetIndex();

	//check if there are records
	if (s_player_record[p_index].records.size() < 1)
		return;
	...
	if (nci)
		correct += nci->GetLatency(FLOW_OUTGOING); // add network latency

	int lerpTicks = TIME_TO_TICKS(g_BacktrackHelper->GetLerpTime()); // calc number of view interpolation ticks - 1

	correct += TICKS_TO_TIME(lerpTicks);

	// check bouns [0,sv_maxunlag]
	correct = clamp(correct, 0.0f, 64.f);

	// correct tick send by player 
	int targettick = G::UserCmdForBacktracking->tick_count - lerpTicks;

	// calc difference between tick send by player and our latency based tick
	float deltaTime = correct - TICKS_TO_TIME(g_BacktrackHelper->GetEstimateServerTickCount() - targettick);
	...
	for (int i = 0; i <= 13; i++)
	{
		pPlayer->SetAbsOrigin(s_player_record[p_index].records[i].m_vecOrigin);
		*(float*)((DWORD)pPlayer + offys.m_flSimulationTime) = s_player_record[p_index].records[i].m_flSimulationTime;
		*(Vector*)((DWORD)pPlayer + 0x110) = s_player_record[p_index].records[i].m_vecVelocity;
		G::UserCmdForBacktracking->tick_count = TIME_TO_TICKS(s_player_record[p_index].records[i].m_flSimulationTime);
...
void CBacktrackHelper::LagCompensate(CBaseEntity* pPlayer)
{
	...
	for (int i = 0; i < 32; i++)
	{
		...
		if (this->PlayerRecord[index].records[i].m_flSimulationTime == NULL)
			continue;

		Vector org_delta = this->PlayerRecord[index].records[i].m_vecOrigin - this->PlayerRecord[index].records[i + 1].m_vecOrigin;
		int LerpTicks = TIME_TO_TICKS(this->GetLerpTime());
		int desired = TIME_TO_TICKS(this->PlayerRecord[index].records[i].m_flSimulationTime) + LerpTicks;
		...
		int lerpTicks = TIME_TO_TICKS(this->GetLerpTime()); // calc number of view interpolation ticks - 1
		...
		// calc difference between tick send by player and our latency based tick
		float deltaTime = correct - TICKS_TO_TIME(this->GetEstimateServerTickCount() - targettick);

		if (fabs(deltaTime) < 0.2f)
		{
			pPlayer->SetAbsOrigin(this->PlayerRecord[index].records[i].m_vecOrigin);
			*(float*)((DWORD)pPlayer + offys.m_flSimulationTime) = this->PlayerRecord[index].records[i].m_flSimulationTime;
			//	*(float*)((DWORD)pPlayer + offys.m_flCycle) = this->PlayerRecord[index].records[i].m_flCycle;
			//	*(int*)((DWORD)pPlayer + offys.m_nSequence) = this->PlayerRecord[index].records[i].m_nSequence;
			*(Vector*)((DWORD)pPlayer + 0x110) = this->PlayerRecord[index].records[i].m_vecVelocity;

			G::UserCmdForBacktracking->tick_count = TIME_TO_TICKS(this->PlayerRecord[index].records[i].m_flSimulationTime);
...
Vector GetHitboxPosition(CBaseEntity* pEntity, int Hitbox)
{
	...
	VMatrix matrix[MAXSTUDIOBONES];

	if (pEntity->SetupBones(matrix, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, g_pGlobals->curtime))
		//if (pEntity->SetupBonesRebuild(128, matrix))
	{
...
bool BackTrack::RunLBYBackTrack(int i, CUserCmd* cmd, QAngle& aimPoint)
{
...
BackTrack* g_backtrack = new BackTrack();