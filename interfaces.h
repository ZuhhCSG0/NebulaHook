...
public:
	...
	bool GetInterfaces()
	{
		...
		g_pInput = *(CInput**)((*(uintptr_t**)g_pClient)[15] + 0x1);
		...
		g_pViewRenderBeams = *(IViewRenderBeams**)(FindPatternIDA("client.dll", "B9 ? ? ? ? A1 ? ? ? ? FF 10 A1 ? ? ? ? B9") + 1);
		
		g_pFireBullet = (C_TEFireBullets*)((DWORD)GetModuleHandle("client.dll") + 0xA5B498);//(FindPatternIDA("client.dll", "8B D1 B8 ? ? ? ? B9") + 0x81);
		g_pClientState = **(CClientState***)(FindPatternIDA("engine", "8B 3D ? ? ? ? 8A F9") + 2);
