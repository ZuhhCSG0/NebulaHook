...
{
	auto fn = (void(*)(void))
		Utilities::Memory::FindPatternIDA(
			"engine.dll",
...
//	SkinsUtil::ParseSkins();
	...
	engineVMT->HookVM((void*)Hooks::IsBoxVisible, 32);
	engineVMT->ApplyVMT();
	...
	clientmodeVMT->HookVM((void*)Hooks::H_GetViewModelFOV, 35);
...
//	modelrenderVMT->ApplyVMT();
	...
	if (!only)
	{
		Utilities::Log("AnimOverlay : %d", Utilities::Memory::FindPatternIDA("client.dll", "55 8B EC 51 53 8B 5D 08 33 C0"));
		only = true;
	}
