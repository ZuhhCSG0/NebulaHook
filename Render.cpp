...
void CRender::Init(IDirect3DDevice9* pDevice)
{
	...
	*/
	...
	D3DXCreateFontA(this->device, this->menu_control_size/*size*/, 0, 500, 1, /*(italic) ? 1 : */0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, "Verdana", &this->Fonts.menu_control);

	return;
}

void CRender::FillARGBTriangle(int x1, int y1, int x2, int y2, int x3, int y3, D3DCOLOR color)
{
	struct Vertex
	{
		float x, y, z, ht;
...
int CRender::GetTextWidth(const char *szText, LPD3DXFONT pFont)
{
	RECT rcRect = { 0,0,0,0 };
	if (pFont)
	{
...
int CRender::GetTextHeight(const char *szText, LPD3DXFONT pFont)
{
	RECT rcRect = { 0,0,0,0 };
	if (pFont)
	{
	...
	// return width
	return rcRect.bottom;
}

void CRender::BorderBoxOutlinedText2(int x, int y, int w, int h, int thickness, D3DCOLOR color, D3DCOLOR outlined, char* meme, LPD3DXFONT font)
{
	int Width = this->GetTextWidth((const char*)meme, font);
	int Height = this->GetTextHeight((const char*)meme, font);

	this->Text(meme, x + w / 2, y - Height / 2, text_alignment::centered, font, false, D3DCOLOR_RGBA(255, 255, 255, 255), D3DCOLOR_RGBA(0,0,0,0));

	this->Line(x, y, x + w / 2 - Width / 1.5, y, outlined);
	this->Line(x + w / 2 + Width / 1.5, y, x + w + 1, y, outlined);
	this->Line(x, y, x, y + h + 1, outlined);
	this->Line(x, y + h, x + w + 1, y + h, outlined);
	this->Line(x + w + 1, y, x + w + 1, y + h + 1, outlined);
}


void CRender::BorderBoxOutlinedText(int x, int y, int w, int h, int thickness, D3DCOLOR color, D3DCOLOR outlined, char* meme, LPD3DXFONT font)
{
	int Width = this->GetTextWidth((const char*)meme, font);
	int Height = this->GetTextHeight((const char*)meme, font);
